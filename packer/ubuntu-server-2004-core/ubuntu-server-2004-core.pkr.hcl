...
variable "vm_boot_iso" {
  description = "Path to the Ubuntu ISO in Proxmox storage (e.g. local:iso/ubuntu-20.04.6-live-server-amd64.iso)."
  type        = string
...
source "proxmox-iso" "ubuntu-2004-core" {
  ...
  # VM General Settings
  vm_id                = var.vm_id
  vm_name              = "template-ubuntu-2004-server-core"
  ...
  # Steps:
  # 1. e: Edit the boot command
  # 2. <down><down><down><end>: Navigate to end of kernel command line
  # 3. Append autoinstall flags (ds=nocloud to use cloud-init ISO)
  # 4. <f10>: Boot with modified parameters
  boot_wait = "5s"
  boot_command = [
    "<wait>e<wait>",
    "<down><down><down><end>",
...
build {
  name    = "ubuntu-2004-core"
  sources = ["source.proxmox-iso.ubuntu-2004-core"]
  ...
  provisioner "shell" {
    inline = [
      ...
      "echo 'datasource_list: [NoCloud, None]' | sudo tee /etc/cloud/cloud.cfg.d/99-pve-datasource.cfg",
      "sudo rm -f /etc/netplan/00-installer-config.yaml",
      "echo \"Ubuntu 20.04 Template by Packer - Creation Date: $(date)\" | sudo tee /etc/issue"
    ]
  }
