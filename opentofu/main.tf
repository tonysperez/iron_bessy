...
locals {
  ubuntu_server_2004_core_resolved = {
    for name, vm in var.ubuntu_server_2004_core_vms : name => merge(vm, {
...
resource "proxmox_virtual_environment_vm" "ubuntu_server_2004_core_vms" {
  for_each = local.ubuntu_server_2004_core_resolved

  name        = each.key
  description = "Cloned from ubuntu-server-2004-core, managed by OpenTofu"
  tags        = ["opentofu", lower(terraform.workspace)]
  node_name   = var.proxmox_node
  pool_id     = var.vm_pool

  clone {
    vm_id = var.template_vm_ids["ubuntu-server-2004-core"]
    full  = true
  }
...
locals {
  ...
  all_vms = merge(
    ...
    local.ubuntu_server_2004_core_resolved,
  )
  ...
  vm_ids = merge(
    ...
    { for k, v in proxmox_virtual_environment_vm.ubuntu_server_2004_core_vms : k => v.vm_id },
  )
}
